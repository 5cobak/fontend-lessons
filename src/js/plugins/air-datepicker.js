...
(function (window, $, undefined) {
  (function () {
    ...
    const defaults = {
      ...
      prevHtml: '<svg><use xlink:href="#arrow_back"</svg>',
      nextHtml: '<svg class="arrow-forward arrow-forward_purple"><use xlink:href="#arrow_forward"</svg>',
      navTitles: {
        days: 'MM <i>yyyy</i>',
        months: 'yyyy',
        years: 'yyyy1 - yyyy2',
  ...
  (function () {
    ...
    const buttonsContainerTemplate = '<div class="datepicker--buttons"><span class="datepicker--button-apply button button_no-bg" data-action="">Применить</span></div>';
    const button = '<span class="datepicker--button button button_no-bg" data-action="#{action}">#{label}</span>';
    const buttonApply = '<span class="datepicker--button">#{label}</span>';
    const { datepicker } = $.fn;
    const dp = datepicker.Constructor;
