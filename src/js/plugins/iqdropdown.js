  ...
  $.fn.iqDropdown = function (options) {
    this.each(function () {
      ...
      function addControls(id, $item) {
        ...
        $buttonsActions.click((e) => {
          e.stopPropagation();
        });
        $buttonCleanOff.click((e) => {
          totalItems = 0;
          itemCount[id] = 0;
...
$(document).ready(() => {
  ...
  // max total items
    ...
    setSelectionText: (itemCount, totalItems) => {
      // line declination function
      const { item1 } = itemCount;
      const { item2 } = itemCount;
      const { item3 } = itemCount;
      function declination(n, text_forms) {
        n = Math.abs(n) % 100; const n1 = n % 10;
        if (n >= 5 && n < 20) { return text_forms[2]; }
        if (n1 > 1 && n1 <= 4) { return text_forms[1]; }
      ...
      const total = [];
      const t1 = `${item1} ${declination(item1, ['спальня', 'спальни', 'спальней', 'спальней'])}`;
      const t2 = `${item2} ${declination(item2, ['кровать', 'кровати', 'кроватей', 'кроватей'])}`;
      const t3 = `${item3} ${declination(item3, ['ванная', 'ванны', 'ванных', 'ванных'])}`;
      if (item1 > 0) {
        total.push(t1);
      }
      if (item2 > 0) {
      ...
      return `${('Спальни, кровати, ванные комнаты').substr(0, 20)}...`;
    },
    controls: {
      counterCls: 'counter',
    },
  ...
  $('.buttons-actions__clean-off').click(() => {
